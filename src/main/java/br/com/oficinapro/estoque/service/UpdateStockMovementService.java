...
@Service
public class UpdateStockMovementService {
    ...
    public UpdateStockMovementService(StockMovementRepository stockMovementRepository,
    ...
    @Transactional
    public StockMovementResponse update(UUID id, StockMovementRequest request) {
        ...
        stockMovementMapper.updateEntity(request, stockMovement);
        stockMovement.setProduct(findProduct(request.productCode()));
        stockMovement.setMovedBy(findUser(request.movedByCode()));
        if (request.movedAt() != null) {
            stockMovement.setMovedAt(request.movedAt());
        }
